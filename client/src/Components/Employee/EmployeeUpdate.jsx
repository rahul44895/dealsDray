...
export default function EmployeeUpdate() {
  ...
  useEffect(() => {
    ...
    if (employee) setEmpDetails(employee);
    else {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (empDetails.f_Mobile.length > 10)
      return alert("Phone Number should be of exactly 10 digits.");
    const res = await updateEmployee(empDetails);
    if (res) {
      setEmpDetails({
        f_Name: "",
        f_Email: "",
  ...
  return (
    <div>
      <h1 className="bg-warning p-1">Employee - Update</h1>
      <form style={{ width: "80%", margin: "auto" }} onSubmit={handleSubmit}>
        <div className="mb-3">
          <img
            src={`${host}/${empDetails.f_Image}`}
            alt="avatar"
            style={{
              height: "100px",
              width: "100px",
        ...
        <div className="mb-3">
          ...
          <select
            ...
            id="f_Designation"
            name="f_Designation"
            value={empDetails.f_Designation}
            onChange={(e) => {
          ...
          <div className="d-flex gap-5">
            <div>
              <input
                ...
                value="MCA"
                className="m-1"
                checked={empDetails.f_Course.includes("MCA")}
                onChange={(e) => {
            ...
            <div>
              <input
                ...
                value="BCA"
                className="m-1"
                checked={empDetails.f_Course.includes("BCA")}
                onChange={(e) => {
                  const { value, checked } = e.target;
                  let updatedCourses;
            ...
            <div>
              <input
                ...
                value="BSC"
                className="m-1"
                checked={empDetails.f_Course.includes("BSC")}
                onChange={(e) => {
                  const { value, checked } = e.target;
                  let updatedCourses;
